...
const init = () => {
        ...
        if (typeof MediaRecorder === "undefined" || !tieneSoporteUserMedia())
            return alert("Your borwser not support this tachnology, please get upload it");
        ...
        const segundosATiempo = numeroDeSegundos => {
            let hrs, min, seg, res;
            hrs = Math.floor(numeroDeSegundos / 3600);
            res = Math.floor(numeroDeSegundos % 3600);
            min = Math.floor(res / 60);
            seg = Math.floor(res % 60);
            return `${hrs}:${min}:${seg}`;
        };
        // Variables "globales"
